	...
	private sealed partial class D2DContextImpl
	{
		...
		public void DrawImage(INativeTexture2D texture, float x, float y, float w, float h, float opacity = 1)
		{
			...
			switch (texture)
			{
				case DX11NV12VideoTexture nv12:
					...
					D2DContext.DrawBitmap(nv12.D2DBitmap, opacity, BitmapInterpolationMode.Linear, destRect);
					return;
				case D2DTexture d2dTex:
					D2DContext.DrawBitmap(d2dTex.Bitmap, opacity, BitmapInterpolationMode.Linear, destRect);
					return;
			}
		}
		...
		public void DrawImage(INativeTexture2D texture, RRect srcRect, float x, float y, float w, float h, float opacity = 1)
		{
			...
			switch (texture)
			{
				case DX11NV12VideoTexture nv12:
					nv12.D2DBitmap ??= CreateNV12D2DBitmap(nv12.OutputRgba, D2DContext);
					D2DContext.DrawBitmap(nv12.D2DBitmap, destRect, opacity, BitmapInterpolationMode.Linear, src);
					return;
				case D2DTexture d2dTex:
		...
		private static ID2D1Bitmap1 CreateNV12D2DBitmap(ID3D11Texture2D rgbaTex, ID2D1DeviceContext ctx)
		{
			using var surface = rgbaTex.QueryInterface<IDXGISurface>();
			return ctx.CreateBitmapFromDxgiSurface(surface, new BitmapProperties1(new Vortice.DCommon.PixelFormat(Format.R8G8B8A8_UNorm,
						Vortice.DCommon.AlphaMode.Premultiplied), 96, 96, BitmapOptions.None));
		}
	}
}